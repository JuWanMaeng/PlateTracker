...
def make_parser():
    ...
    parser.add_argument(
        "demo", choices=["image", "video", "webcam"], help="demo type, eg. image, video and webcam"
    )

    parser.add_argument(
        "--path", default="input_video/video.mp4", help="path to images or video"
    )
    parser.add_argument("--camid", type=int, default=0, help="camera ID for webcam mode")
    parser.add_argument(
        "--save_result",
        action="store_true",
...
def imageflow_demo(predictor, vis_folder, current_time, args):
    ...
    while True:
        ...
        if ret_val:
            outputs, img_info = predictor.inference(frame, timer, args) # model 결과 예측
            if outputs[0] is not None:
                online_targets = tracker.update(outputs[0], [img_info['height'], img_info['width']], None) # tracking 시작
                online_tlwhs = []
                online_ids = []
                online_scores = []
                online_xyxy = []
                for t in online_targets:
                    ...
                    if tlwh[2] * tlwh[3] > args.min_box_area and not vertical:
                        ...
                        # 50 frame 마다 OCR을 진행합니다.
                        if frame_id % 50 == 0: 
                            ocrbox = tuple(map(int, (xyxy[0], xyxy[1], xyxy[2], xyxy[3]))) # object의 좌표
...
if __name__ == "__main__":
    ...
    exp = YOLOVideoProcessor(model_path='./pretrained/algorithm2_original.engine', margin_ratio=0.2, confidence_threshold=0.3, bbox_margin_w = 0.5, bbox_margin_h = 2) # tracking 을 위한 bbox w, h 에 각각 50%, 200% margin 을 주었습니다. 
